...
exports.getRecyclerStats = exports.updateRecyclingStatus = exports.confirmReceipt = exports.listShipments = void 0;
...
const listShipments = async (req, res) => {
    try {
        ...
        const filterQuery = { recyclingCenter: req.user.id };
        if (status) {
        ...
        const shipments = await PlasticRequest_1.default.find(filterQuery)
...
exports.updateRecyclingStatus = updateRecyclingStatus;
...
const getRecyclerStats = async (req, res) => {
    try {
        ...
        const recycler = await User_1.default.findById(req.user.id);
        if (!recycler)
            return res.status(404).json({ message: 'Recycling center not found.' });
        const incomingCount = await PlasticRequest_1.default.countDocuments({
            recyclingCenter: req.user.id,
            status: 'picked_up'
        });
        const receivedCount = await PlasticRequest_1.default.countDocuments({
            recyclingCenter: req.user.id,
            status: 'received'
        });
        const recycledCount = await PlasticRequest_1.default.countDocuments({
            recyclingCenter: req.user.id,
            status: 'recycled'
        });
        // Calculate total weight recycled
        const totalRecycledWeight = await PlasticRequest_1.default.aggregate([
...
exports.getRecyclerStats = getRecyclerStats;
